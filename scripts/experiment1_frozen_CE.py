"""
Experiment 1 — CONCH + MLP Classifier
---------------------------------------
Baseline experiment: frozen CONCH features + MLP head
Standard cross-entropy loss, no noise handling.

This establishes the baseline number to compare against:
...
Results saved to: ./results/experiment1_results.txt
"""
...
# ── Hyperparameters ───────────────────────────────────────────────────────────
FEATURE_DIM  = 512       # CONCH ViT-B-16 output dimension
NUM_CLASSES  = 4
HIDDEN_DIM   = 256
DROPOUT      = 0.3
BATCH_SIZE   = 128
...
class MLPClassifier(nn.Module):
    """
    Two-layer MLP classifier head on top of frozen CONCH features.
    Architecture mirrors Ke et al. (2025): hidden->64->classes
    but with wider hidden layer given richer CONCH features.
    """
...
def load_features(path: str, split_name: str):
    """Load cached CONCH features and labels from disk."""
...
def save_results(results_text: str):
    ...
    path = os.path.join(RESULTS_DIR, "experiment1_results.txt")
...
def main():
    ...
    print("=" * 60)
    print("  Experiment 1 — CONCH + MLP, No Noise Handling")
    print("=" * 60)
    ...
    results_lines.append("  Experiment 1 — CONCH + MLP, No Noise Handling")
    results_lines.append("=" * 60)
...
── Comparison Against Published Baselines ──
  ...
  CONCH + MLP (Experiment 1, ours)    {acc:.2f}%   {f1:.2f}%
"""
    print(comparison)
    ...
    ckpt_path = os.path.join(RESULTS_DIR, "experiment1_best_model.pt")
    torch.save({
        'epoch':       best_epoch,
