...
EPOCHS         = 150
WARMUP_EPOCHS  = 15
LR_ENCODER     = 1e-4     # CONCH unfrozen blocks
...
LORA_ALPHA      = 16     # scaling factor — keep equal to r for neutral scaling
LORA_DROPOUT    = 0.1    # dropout on LoRA layers
# Target modules — UPDATE THESE after running discover_attention_layers()
# Common timm ViT names: 'qkv', 'proj', or separate 'q', 'k', 'v'
...
def load_conch_lora(weights_path: str, device: str):
    """
    Load CONCH ViT-B/16 and apply LoRA adapters to all attention blocks.

    Replaces manual layer-freezing with LoRA for three reasons:
      1. All 12 blocks adapt — lower blocks learn H&E texture dynamics
      2. Low-rank bottleneck (r=16) prevents overfitting on 4,021 samples
      3. ~1.2M trainable params vs ~46M — fits comfortably in 8GB VRAM

    IMPORTANT: Run with DISCOVER_LAYERS=True first to confirm target
    module names match your installed CONCH/timm version.
    """
    ...
    # ── Optional: discover layer names (set True on first run) ────────────────
    DISCOVER_LAYERS = False
    if DISCOVER_LAYERS:
        print("\nAttention-related Linear layers in CONCH trunk:")
        for name, module in encoder.named_modules():
            if isinstance(module, nn.Linear):
                if any(x in name for x in
                       ['attn', 'qkv', 'proj', 'q_', 'v_', 'q.', 'v.']):
                    print(f"  {name}: {module.in_features}→{module.out_features}")
        print("\nUpdate LORA_TARGET_MODULES then set DISCOVER_LAYERS=False\n")
        raise SystemExit("Discovery mode — update config and rerun.")
    ...
    lora_config = LoraConfig(
        ...
        bias             = "none",
        # No task_type — we are wrapping a bare nn.Module not a HF model
    )
    ...
    with torch.no_grad():
        ...
        out   = encoder(dummy)
        # peft wraps the model — output format may change
        # handle tuple, list, or direct tensor
...
class DualEncoderModel(nn.Module):
    ...
    def forward(self, uni_feat: torch.Tensor,
        ...
        # Handle peft wrapper output format
        if isinstance(out, (tuple, list)):
            conch_feat = out[0]
        elif hasattr(out, 'last_hidden_state'):
            conch_feat = out.last_hidden_state[:, 0]
        else:
            conch_feat = out                         # [B, 512]
...
def main():
    ...
    # ── Load CONCH ────────────────────────────────────────────────────────────
    conch_encoder, conch_preprocess, conch_dim = load_conch_lora(
        CONCH_WEIGHTS, DEVICE
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if f1 > best_f1:
            ...
            }, os.path.join(RESULTS_DIR, "phase2_best_f1_selfLoRA.pt"))
        ...
        if acc > best_acc:
            ...
            }, os.path.join(RESULTS_DIR, "phase2_best_acc_selfLoRA.pt"))
    ...
    print(f"\nTraining Complete!")
    ckpt = torch.load(os.path.join(RESULTS_DIR, "phase2_best_acc_selfLoRA.pt"))
    best_acc_f1 = ckpt['f1']
...
"""
    ...
    results_path = os.path.join(RESULTS_DIR, "phase2_results_selfLoRA.txt")
