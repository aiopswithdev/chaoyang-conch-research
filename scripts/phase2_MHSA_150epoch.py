...
# ── Paths ─────────────────────────────────────────────────────────────────────
CONCH_WEIGHTS  = "pytorch_model.bin"
TRAIN_DIR      = "Chaoyang/archive/organized_images_train"
TEST_DIR       = "Chaoyang/archive/organized_images_test"
UNI_TRAIN_CACHE = "Chaoyang/features/uni_train_features.pt"
UNI_TEST_CACHE  = "Chaoyang/features/uni_test_features.pt"
RESULTS_DIR    = "./results"

# ── Hyperparameters ───────────────────────────────────────────────────────────
NUM_CLASSES    = 4
COMMON_DIM     = 512      # projection dimension for cross-attention
...
NUM_TOKENS     = 8        # concat vector is split into this many tokens for MHSA
                          # token_dim = common_dim // num_tokens = 64
                          # head_dim  = token_dim  // num_heads  = 16
MHSA_LAYERS    = 2        # number of transformer encoder layers in fusion head
...
class ConcatMHSAHead(nn.Module):
    """
    Concat → Linear → MHSA fusion head.

    Step 1 — Concatenate raw modality features:
        [B, 1024] ++ [B, 512] → [B, 1536]

    Step 2 — Single linear transform into a shared feature space:
        Linear(1536, common_dim) + LayerNorm → [B, common_dim]
        Both modalities are now in the same representation space.

    Step 3 — Split into tokens for MHSA:
        [B, common_dim] → [B, num_tokens, token_dim]
        where token_dim = common_dim // num_tokens
        Each token is a learned sub-representation of the fused vector.

    Step 4 — Multi-Head Self-Attention (Pre-LN TransformerEncoder):
        Tokens attend to each other → [B, num_tokens, token_dim]

    Step 5 — Flatten + MLP classifier → [B, num_classes]
    """
    def __init__(self, uni_dim: int, conch_dim: int,
                 ...
                 num_tokens: int = 4, num_layers: int = 2):
        super().__init__()
        assert common_dim % num_tokens == 0, \
            f"common_dim ({common_dim}) must be divisible by num_tokens ({num_tokens})"
        
        self.noise_stddev = noise_stddev
        self.num_tokens   = num_tokens
        self.token_dim    = common_dim // num_tokens  # 256 // 4 = 64

        # Step 1+2: concat + project into shared feature space
        self.proj      = nn.Linear(uni_dim + conch_dim, common_dim)
        self.proj_norm = nn.LayerNorm(common_dim)

        # Step 4: MHSA over the token sequence
        # d_model = token_dim = 64, nhead = 4 → head_dim = 16
        ...
        # Step 5: flatten attended tokens → MLP
    ...
    def forward(self, uni_feat: torch.Tensor,
        ...
        # Step 1: concat → [B, 1536]
        x = torch.cat([uni_feat, conch_feat], dim=-1)

        # Step 2: project to shared space → [B, common_dim]
        x = self.proj_norm(self.proj(x))

        # Step 3: split into tokens → [B, num_tokens, token_dim]
        x = x.view(x.size(0), self.num_tokens, self.token_dim)

        # Step 4: MHSA — tokens attend to each other → [B, num_tokens, token_dim]
        x = self.transformer(x)

        # Step 5: flatten → [B, common_dim] → classifier
        x = x.flatten(1)
...
def main():
    ...
    encoder_params = [p for p in model.conch_encoder.parameters()
                      if p.requires_grad]
    fusion_params  = list(model.fusion_head.proj.parameters())         + \
                     list(model.fusion_head.proj_norm.parameters())     + \
                     list(model.fusion_head.transformer.parameters())
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if f1 > best_f1:
            ...
            }, os.path.join(RESULTS_DIR, "phase2_best_f1_MHSA_150.pt"))
        ...
        if acc > best_acc:
            ...
            }, os.path.join(RESULTS_DIR, "phase2_best_acc_MHSA_150.pt"))
    ...
    print(f"\nTraining Complete!")
    ckpt = torch.load(os.path.join(RESULTS_DIR, "phase2_best_acc_MHSA_150.pt"))
    best_acc_f1 = ckpt['f1']
...
"""
    ...
    results_path = os.path.join(RESULTS_DIR, "phase2_results_MHSA_150.txt")
